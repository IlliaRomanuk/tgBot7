# handlers/problem_handler.py - COMPLETE REFACTOR: Production-ready problem handler
import logging
...
async def start_problem_flow(message: types.Message, state: FSMContext):
    """Start problem submission flow - REFACTORED with robust filter and diagnostics"""
    # Comprehensive diagnostic logging
    print(f"🔍 [PROBLEM HANDLER] Raw message: {repr(message.text)}")
    print(f"🔍 [PROBLEM HANDLER] Lowercase: {repr(message.text.lower())}")
    print(f"🔍 [PROBLEM HANDLER] Contains 'проблем'? {'проблем' in message.text.lower()}")
    print(f"🔍 [PROBLEM HANDLER] Type: {type(message.text)}")
    print(f"🔍 [PROBLEM HANDLER] Length: {len(message.text) if message.text else 'None'}")
    ...
    try:
        ...
        # Set FSM state
        logger.info("⚙️ Setting FSM state to ProblemFlow.severity")
...
async def process_problem_severity(message: types.Message, state: FSMContext):
    """Process problem severity selection - REFACTORED"""
    # Diagnostic logging
    print(f"🔍 [PROBLEM SEVERITY] Raw message: {repr(message.text)}")
    print(f"🔍 [PROBLEM SEVERITY] Expected: {['🚨 Критическая проблема', '⚠️ Серьезная проблема', '❗️ Умеренная проблема', '⚡️ Малозначительная проблема', '🔧 Мелкая проблема']}")
...
async def process_problem_description(message: types.Message, state: FSMContext):
    """Process problem description - REFACTORED"""
    # Diagnostic logging
    print(f"🔍 [PROBLEM DESCRIPTION] Raw message: {repr(message.text)}")
    print(f"🔍 [PROBLEM DESCRIPTION] Length: {len(message.text) if message.text else 'None'}")
...
async def process_meeting_needed(message: types.Message, state: FSMContext):
    """Process meeting needed selection - REFACTORED"""
    # Diagnostic logging
    print(f"🔍 [MEETING NEEDED] Raw message: {repr(message.text)}")
    print(f"🔍 [MEETING NEEDED] Expected: {['Да', 'Нет']}")
...
# REFACTORED: No individual debug handlers - moved to global handler in main.py
# This prevents message interception and ensures proper routing
