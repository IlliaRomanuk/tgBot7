# handlers/test_handler.py - COMPLETE REFACTOR: Production-ready test handler
import logging
...
async def start_test_flow(message: types.Message, state: FSMContext):
    """Start daily test flow - REFACTORED with robust filter and diagnostics"""
    ...
    try:
        ...
        # Set FSM state
        logger.info("⚙️ Setting FSM state to TestFlow.mood")
...
async def process_mood(message: types.Message, state: FSMContext):
    """Process mood response - REFACTORED"""
    # Diagnostic logging
    print(f"🔍 [MOOD RESPONSE] Raw message: {repr(message.text)}")
    print(f"🔍 [MOOD RESPONSE] Expected values: {['😀 Отлично', '🙂 Хорошо', '😐 Нормально', '😟 Плохо']}")
    ...
    try:
        ...
        logger.info("⚙️ Setting state to TestFlow.motivation")
...
async def process_motivation(message: types.Message, state: FSMContext):
    """Process motivation response - REFACTORED"""
    # Diagnostic logging
    ...
    try:
        ...
        # Set next state and send question
        logger.info("⚙️ Setting state to TestFlow.problems")
...
async def process_problems(message: types.Message, state: FSMContext):
    """Process problems response - REFACTORED"""
    # Diagnostic logging
...
async def send_daily_test(bot):
    """Send daily test to all users - REFACTORED"""
    try:
