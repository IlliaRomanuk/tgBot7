# handlers/problem_handler.py - PRODUCTION READY: Robust filters and proper FSM state management
import logging
...
# PRODUCTION READY: Robust filter for problem button - handles all variations including case-insensitive
# Uses F.text.lower().contains("проблем") to catch:
# - "Сообщить о проблеме" (the actual button text users see)
# - "проблема" (partial match)
# - "проблем" (root word)
# - Case variations: "ПРОБЛЕМА", "проблема", "сообщить о проблеме", etc.
@router.message(F.text.lower().contains("проблем"))
async def start_problem_flow(message: types.Message, state: FSMContext):
    """Start problem submission flow - PRODUCTION READY: Robust filter for all variations"""
    try:
