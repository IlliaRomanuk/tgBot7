# handlers/idea_handler.py - COMPLETE REFACTOR: Production-ready idea handler
import logging
...
async def start_idea_flow(message: types.Message, state: FSMContext):
    """Start idea submission flow - REFACTORED with robust filter and diagnostics"""
    ...
    try:
        ...
        # Set FSM state
        logger.info("⚙️ Setting FSM state to IdeaFlow.priority")
...
async def process_idea_priority(message: types.Message, state: FSMContext):
    """Process idea priority selection - REFACTORED"""
    # Diagnostic logging
    print(f"🔍 [IDEA PRIORITY] Raw message: {repr(message.text)}")
    print(f"🔍 [IDEA PRIORITY] Expected: {['🌟 Высший', '🔥 Высокий', '💡 Средний', '🌱 Низкий', '⏳ Минимальный']}")
...
async def process_idea_type(message: types.Message, state: FSMContext):
    """Process idea type selection - REFACTORED"""
    # Diagnostic logging
    print(f"🔍 [IDEA TYPE] Raw message: {repr(message.text)}")
    print(f"🔍 [IDEA TYPE] Expected: {['улучшение работы', 'новое направление']}")
...
async def process_idea_description(message: types.Message, state: FSMContext):
    """Process idea description - REFACTORED"""
    # Diagnostic logging
