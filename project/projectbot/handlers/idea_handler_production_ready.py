# handlers/idea_handler.py - PRODUCTION READY: Robust filters and proper FSM state management
import logging
...
# PRODUCTION READY: Robust filter for idea button - handles all variations including case-insensitive
# Uses F.text.lower().contains("иде") to catch:
# - "Предложить идею" (the actual button text users see)
# - "идею" (partial match)
# - "идея" (variation)
# - Case variations: "ИДЕЮ", "идею", "предложить идею", etc.
@router.message(F.text.lower().contains("иде"))
async def start_idea_flow(message: types.Message, state: FSMContext):
    """Start idea submission flow - PRODUCTION READY: Robust filter for all variations"""
    try:
