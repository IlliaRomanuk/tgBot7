# handlers/problem_handler.py - WORKING VERSION with diagnostics
import logging
...
@router.message(lambda m: m.text and "проблем" in m.text.lower())
async def start_problem_flow(message: types.Message, state: FSMContext):
    """Start problem submission flow"""
    print("🚨 PROBLEM HANDLER TRIGGERED - FILTER MATCHED!")
...
async def process_problem_severity(message: types.Message, state: FSMContext):
    ...
    print("🚨 PROBLEM SEVERITY HANDLER TRIGGERED!")
...
async def process_problem_description(message: types.Message, state: FSMContext):
    ...
    print("📝 PROBLEM DESCRIPTION HANDLER TRIGGERED!")
...
async def process_meeting_needed(message: types.Message, state: FSMContext):
    ...
    print("🤝 MEETING NEEDED HANDLER TRIGGERED!")
...
print("✅ problem_handler.py loaded - Router instance created")
