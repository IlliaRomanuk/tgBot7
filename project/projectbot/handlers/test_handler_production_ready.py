# handlers/test_handler.py - PRODUCTION READY: Robust filters and proper FSM state management
import logging
...
# PRODUCTION READY: Robust filter for test button - handles "Пройти опрос", "Пройти тест", and all variations
# Uses F.text.lower().contains("пройти") to catch:
# - "Пройти опрос" (the actual button text users see)
# - "Пройти тест" (the expected text)
# - "пройти" (partial match)
# - Case variations: "ПРОЙТИ", "пройти", etc.
@router.message(F.text.lower().contains("пройти"))
async def start_test_flow(message: types.Message, state: FSMContext):
    """Start daily test flow - PRODUCTION READY: Robust filter for all button variations"""
    ...
    try:
        # PRODUCTION READY: Set FSM state correctly
        logger.info("Setting FSM state to TestFlow.mood")
        await state.set_state(TestFlow.mood)
        logger.info(f"State after setting: {await state.get_state()}")
        
        # Send first question
        logger.info("Sending mood question")
...
async def process_mood(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        logger.info("Setting state to TestFlow.motivation")
...
async def process_motivation(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        # Set next state and send question
        logger.info("Setting state to TestFlow.problems")
...
# PRODUCTION READY: Removed debug handler - moved to global handler in main.py
# Individual debug handlers were causing interference with message routing
# The global debug handler in main.py will now handle truly unhandled messages
# without causing NameError or message interception
