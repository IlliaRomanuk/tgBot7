# main.py - FIXED: Correct router order and global debug handler
import asyncio
import logging
import sys
...
async def main():
    """Main bot function - FIXED: Correct router registration order"""
    try:
        logger.info("=" * 50)
        ...
        # 2. Specific flows (medium priority)
        ...
        # 3. FIXED: Global debug handler (LOWEST priority - only catches truly unhandled)
        @dp.message()
        async def global_debug_handler(message: types.Message):
            """Global debug handler - ONLY triggers if no other handler matches"""
