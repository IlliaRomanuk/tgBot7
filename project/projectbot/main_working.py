# main.py - FIXED: Global debug handler moved outside main() function
import asyncio
import logging
import sys
...
# Global debug handler (lowest priority) - OUTSIDE main function, registered LAST
@dp.message()
...
async def main():
    """Main bot function - FIXED: Global debug handler moved outside"""
    try:
        logger.info("=" * 60)
        ...
        print("  1. start_router (/start, /help)")
        print("  2. test_router (пройти)")
        print("  3. idea_router (иде)")
        print("  4. problem_router (проблем)")
        print("  5. global_debug_handler (lowest priority)")
